...
ينشئ مالك المشروع مشروع Supabase جديدًا باسم واضح مثل `bisis-platform-staging` داخل المؤسسة الصحيحة، مع منطقة مناسبة وخطة تكفي للاختبار. يجب تسجيل project ref الجديد، المنطقة، وخطة المشروع في سجل داخلي غير سري. لا يُنسخ أي service-role secret من Production إلى Staging.
